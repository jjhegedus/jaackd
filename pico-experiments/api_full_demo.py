...
async def handle_request(reader, writer):
    try:
        ...
        print("******* Raw Request Begin *****/n/n")
        print(raw_request)
        print("******* Raw Request End *****/n/n")
        ...
        if request.url_match("/api"):
            ...
            if action == 'readPot':
            ...
            elif action == 'turnInternalLedOn':
                # turn on the internal led
                # returns json object with internal led state
                internal_led = Pin("LED", Pin.OUT)
                internal_led.value(1)
                int_led_state = internal_led.value()

                status = 'OK'
                led_state = {
                    'int_led_state': int_led_state
                }
                #IoHandler.set_rgb_leds(rgb_red, rgb_green, rgb_blue)
                response_obj = {
                    'status': status,
                    'internal_led_state': led_state
                }
                response_builder.set_body_from_dict(response_obj)
                print("turned on internal LED")
                ...
                response_builder.set_body_from_dict(response_obj)
                print("turned off internal LED")

            elif action == 'getInternalLedState':
                # returns json object with internal led state
                internal_led = Pin("LED", Pin.OUT)
                internal_led_state = internal_led.value()
        ...
        # try to serve static file
            ...
            print("/api not matched, attempting to load file:", request.url)
            response_builder.serve_static_file(request.url, "/main.html")
...
async def main():
    ...
    server = uasyncio.start_server(handle_request, "0.0.0.0", 3030)
